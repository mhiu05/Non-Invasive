...
def _get_trainer(config, data_loader_dict) -> BaseTrainer:
    """Instantiate the correct trainer for the configured model."""
...
def train_and_test(config, data_loader_dict):
    """Trains the model."""
    model_trainer = _get_trainer(config, data_loader_dict)
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        elif config.TEST.DATA.DATASET == "CustomBigSmall":
            test_loader = data_loader.CustomBigSmallLoader.CustomBigSmallLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS and iBVP.")
